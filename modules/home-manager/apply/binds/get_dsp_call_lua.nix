...
let
    # Strip all the noise from bind.dsp, leaving only the one the user chose
    filteredDsp = filterNullsRecursive bind.dsp;
...
if (firstLevelName == "raw_lua") then
    lib.mkLuaInline bind.dsp.${firstLevelName}.code
else
    let
        # === Import arguments ===
